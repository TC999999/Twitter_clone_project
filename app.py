...
APP_SECRET_KEY="526456u789OIUYT158dgsfhjklOIPUYTRE"
...
def remove_like_from_post(msg_id):
    """User unlikes a message on user's likes page"""
    if not g.user:
        flash("Access unauthorized.", "danger")
        return redirect("/")
...
def profile():
    ...
    print(form.username)
    if form.validate_on_submit():
        if not str(form.password.data):
            flash("Please Input Password.", "danger")
        ...
        if not str(form.image_url.data):
            g.user.image_url = "/static/images/default-pic.png"
        else:
            g.user.image_url = form.image_url.data

        if not str(form.header_image_url.data):
